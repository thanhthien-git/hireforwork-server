...
func setUpCareerRoutes(router *mux.Router, handler *handlers.Handler) {
	...
	// Additional Routes
	router.HandleFunc("/careers/savedjobs/{careerID}", handlers.GetSavedJobs).Methods("GET")
	router.HandleFunc("/careers/viewedjobs/{careerID}", handlers.GetViewedJobs).Methods("GET") 
